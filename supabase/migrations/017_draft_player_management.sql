-- 017_draft_player_management.sql
-- Edit and delete player entries while a game is still in draft.

ALTER TYPE audit_action ADD VALUE IF NOT EXISTS 'entry_deleted';
...
BEGIN
  ...
  IF v_game.status != 'draft' THEN
    RAISE EXCEPTION 'Can only edit players in draft games';
  END IF;
...
BEGIN
  ...
  SELECT * INTO v_game FROM games WHERE id = v_entry.game_id FOR UPDATE;
  IF v_game.status != 'draft' THEN
    RAISE EXCEPTION 'Can only delete players from draft games';
  END IF;
